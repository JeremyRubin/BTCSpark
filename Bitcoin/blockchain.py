...
class TransactionOutput(object, HasParent):
    ...
    def __repr__(self):
        return "<%s instance at %s: %s>"%(self.__class__.__name__,id(self), "btc <%s>, script <%s>"%(self.value/10e7, self.pk_script_parsed()))
    def to_dict(self):
...
class Blocks(object):
    @staticmethod
    def of_buffer(data, offset, size):
        offset = 0
        while offset != size:
            #TODO: What if the magic is not there?
            block, offset = Block.of_buffer(data,offset + 8) # unhex is smaller
            yield block
        raise StopIteration
